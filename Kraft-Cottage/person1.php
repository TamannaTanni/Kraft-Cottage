...
<div class="header_bg">
 ...
 {
   $id = $_GET['remove'];
   $sql1 = "Delete from susers where sid = $id";
...
<!-- products -->
		...
		<div class="container">
          ...
          <tbody style=" font-size:20px; border: 3px solid #18C9D2; ">
                  ...
                  <th><center><strong>Brand</strong></center></th>
          								...
          								<?php
          								include 'connection.php';
          								$s = " SELECT sid FROM susers";
                          $sql="SELECT * FROM products where p_sid==$s";
          								if (!$conn) {
                          ...
                          <td width=10%><strong>'.$row["p_brand"].'</strong></td>
          								...
          								{
          									echo "<tr><td colspan=4><center><a href='#'><h1>No Product Uploaded<h1></a></center></td></tr>";
          								}
          								?>
          </tbody>
          </table>
  ...
  <div class="col-md-4 products-grid-right">
              ...
              include 'connection.php';
              {
                $sql = " SELECT * FROM susers WHERE sid = $id";
                if (!$conn) {
                  die("Connection failed: " . mysqli_connect_error());
                }
                $result = mysqli_query($conn, $sql);
                if (mysqli_num_rows($result) > 0)
                {
                  ...
                  {
                    echo '
                        <h1>Name:	'.$row["susername"].'</h1>
                        <h2>Email:	'.$row["semail"].'</h2>
                        <h2>Mobile:	'.$row["smobile"].'</h2>
                        <h2>Password:	'.$row["spassword"].'</h2>
                        <h2>Address:	'.$row["sadd"].'</h2>
                        <button><a href="person.php?remove='.$row["sid"].'" class="btnRemoveAction"> Account Delete </a></button>';
                 }
                }
            }
              ?>
